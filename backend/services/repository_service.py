"""Repository service: listing, detail and stats workflows."""
...
async def get_repository_by_id(db: AsyncSession, repo_id: int):
    result = await db.execute(select(Repository).where(Repository.id == repo_id))
    return result.scalar_one_or_none()


async def backfill_truncated_readme_if_needed(db: AsyncSession, repo: Repository) -> None:
    ...
    syncer = GitHubSyncer(settings.github_token)
...
async def get_repository_detail(db: AsyncSession, repo_id: int):
    repo = await get_repository_by_id(db, repo_id)
    if not repo:
        return None
    await backfill_truncated_readme_if_needed(db, repo)
    return to_repo_out(repo)


async def get_repository_stats(db: AsyncSession) -> dict:

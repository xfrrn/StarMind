...
from core.llm.client import create_chat_completion
...
async def generate_chat_answer(query: str, repos: list[dict[str, Any]]) -> str:
    prompt = CHAT_RESPONSE_PROMPT.format(
        query=query,
        repos_context=build_repositories_context(repos),
    )
    try:
